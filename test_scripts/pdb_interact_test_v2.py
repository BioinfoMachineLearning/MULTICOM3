...
from bml_casp15.complex_alignment_generation import pdb_interact_v2
...
if __name__ == '__main__':
    ...
    pdb_interact_runner = pdb_interact_v2.PDB_interact_v2(params['uniprot2pdb_mapping_file'])
    pdb_interact_runner.load_data()
    pair_ids = pdb_interact_runner.get_interactions([aln_1, aln_2])

    sequences_full, sequences_monomers, pair_ids = write_concatenated_alignment(pair_ids, [aln_1, aln_2])

    pair_ids.to_csv(f"{outdir}/pdb_interact.csv", index=False)
    print(pair_ids)

    complex_ailgnment_file = f"{outdir}/pdb_interact.a3m"
