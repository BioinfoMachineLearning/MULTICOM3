...
if __name__ == '__main__':
    ...
    pair_ids = UNIPROT_distance_v2.get_interactions([aln_1, aln_2])

    sequences_full, sequences_monomers, pair_ids = write_concatenated_alignment(pair_ids, [aln_1, aln_2])

    pair_ids.to_csv(f"{outdir}/uniprot_dist_interact.csv", index=False)
    print(pair_ids)

    complex_ailgnment_file = f"{outdir}/uniprot_dist_interact.a3m"
