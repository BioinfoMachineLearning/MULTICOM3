...
def evaluate_dimer(inparams):
    ...
    model_count = 0
    target_model_dockq = pd.DataFrame(columns=['dockq', 'tmscore1', 'tmscore2', 'lddt', 'avg_l2_prob', 'avg_l5_prob', 'ptmscore'])
    corr_summary = pd.DataFrame(columns=['dockq', 'tmscore1', 'tmscore2', 'lddt', 'avg_l2_prob', 'avg_l5_prob', 'ptmscore'])
    all_models_path = {}
    for method in methods:
        ...
        if not complete_result(f'{args.output_dir}/{dimer}/{method}'):
            ...
            bfail = False
            method_model_dockq = pd.DataFrame(columns=['dockq', 'tmscore1', 'tmscore2', 'lddt', 'avg_l2_prob', 'avg_l5_prob', 'ptmscore'])
            for i in range(1, 6):
                ...
                split_pdb(model, [f'{outputdir}/{dimer}/{method}/u_model_{i}_s/B.pdb', f'{outputdir}/{dimer}/{method}/u_model_{i}_s/C.pdb'])
            ...
            if bfail:
                ...
                tmscore_argmax = method_model_dockq[
                    method_model_dockq.ptmscore == method_model_dockq.ptmscore.max()].index
                ...
                for index, row in average_pd.iterrows():
                    average_scores += [(float(row['ptmscore']) + float(row['lddt'])/100 + float(row['pairwise_score']))/3]
    ...
    if model_count > 0:
        ...
        sort_tmscore_pd = target_model_dockq.sort_values(by=['ptmscore'], ascending=False, ignore_index=True)
...
if __name__ == '__main__':
    ...
    dockq_program = '/home/casp15/BML_CASP15/tools/DockQ/DockQ.py'
    tmscore_program = '/home/casp15/BML_CASP15/tools/TMscore'
    ...
    corr_summary = pd.DataFrame(columns=['dockq', 'lddt', 'avg_l2_prob', 'avg_l5_prob', 'tmscore1', 'tmscore2'])
    ...
    for line in open(args.dimerlist).readlines():
        dimer = line.rstrip('\n')
        process_list.append([dimer, args.atom_dir, args.output_dir, args.tmpdir, dockq_program, tmscore_program])
