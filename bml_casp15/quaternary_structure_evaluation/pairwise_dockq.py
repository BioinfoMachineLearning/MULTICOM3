...
def run_command(inparams):
    dockq_program, input_dir, pdb1, pdb2 = inparams
    cmd = f"python {dockq_program} -short " + input_dir + '/' + pdb1 + " " \
    ...
    try:
        ...
        print(e)
    return pdb1, pdb2, score_default
    ...
    def run(self, input_dir):
        ...
        for i in range(len(pdbs)):
            for j in range(i + 1, len(pdbs)):
                pdb1 = pdbs[i]
                pdb2 = pdbs[j]
                process_list.append([self.dockq_program, input_dir, pdb1, pdb2])
