...
class Species_interact_v2:
    ...
    def load_monomer_info(alignment):
        ...
        annotation = Species_interact_v2.extract_header_annotation(alignment)

        annotation_table = Species_interact_v2.read_species_annotation_table(annotation)
        ...
        most_similar_in_species = Species_interact_v2.most_similar_by_organism(similarities, annotation_table)

        paralogs = Species_interact_v2.find_paralogs(alignment.main_id, annotation_table, similarities, 0.95)

        most_similar_in_species = Species_interact_v2.filter_best_reciprocal(alignment, paralogs, most_similar_in_species)

        return annotation_table, most_similar_in_species

    def get_interactions_from_two_alignment(alignment1, alignment2, suffix):

        annotation_table_1, most_similar_in_species_1 = Species_interact_v2.load_monomer_info(alignment1)
        ...
        if annotation_table_1 is None or annotation_table_2 is None:
            ...
            species_intersection = most_similar_in_species_1.merge(
                ...
                suffixes=(f"_{suffix}", f"_{suffix + 1}")
            )
    ...
    def get_interactions(alignments):
        ...
        for i in range(0, len(alignments) - 1):
            curr_df = Species_interact_v2.get_interactions_from_two_alignment(alignments[i], alignments[i + 1], suffix=i + 1)
            if prev_df is None:
                prev_df = curr_df
            else:
