...
class Multimer_iterative_refinement_pipeline:
    ...
    def concatenate_msa_and_templates(self,
        ...
        for chain_idx, chain_id in enumerate(chain_id_map):
            ...
            with open(start_multimer_msa + '.temp', 'w') as fw:
                fw.write('\n'.join(fasta_chunks) + '\n')
            combine_a3ms([f"{start_multimer_msa}.temp", start_multimer_msa],
    ...
    def search(self, fasta_file, default_model_dir, input_pdb_dir, outdir, native_pdb_dir=""):
        ...
        for i in range(0, 5):
            ...
            for num_iteration in range(self.max_iteration):
                ...
                if num_iteration == 0:
                    ...
                    for chain_idx, chain_id in enumerate(chain_id_map):
                        os.system(f"cp {default_model_dir}/msas/{chain_id_map[chain_id].description}.paired.a3m "
                                  f"{start_msa_path}/{chain_id_map[chain_id].description}.start.multimer.a3m")

                        os.system(f"cp {default_model_dir}/msas/{PDB_CHAIN_IDS[chain_idx]}/monomer_final.a3m "
