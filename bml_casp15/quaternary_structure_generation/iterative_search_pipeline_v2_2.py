...
class Multimer_iterative_generation_pipeline:
    ...
    def concatenate_msa_and_templates(self,
        ...
        complex_templates_df = check_and_rank_complex_templates(chain_id_map=chain_id_map,
        ...
        if len(complex_templates_df) < self.max_template_count:
            ...
            for template_result, chain_id in zip(template_results, chain_id_map):
                if check_and_rank_monomer_templates_local_and_global(template_result=template_result,
            ...
            for num_iteration in range(self.max_iteration):
                ...
                if not complete_result(out_model_dir):
                    ...
                    if len(pd.read_csv(template_files[0], sep='\t')) == 0:
                        print(
                            f"Cannot find any templates for {'_'.join([chain_id_map[chain_id].description for chain_id in chain_id_map])} in iteration {num_iteration + 1}")
                        break
