...
class Tertiary_structure_evaluation_pipeline:
    ...
    def __init__(self, params, run_methods=["alphafold", "apollo", "enQA"]):
    ...
    def process(self, targetname, fasta_file, monomer_model_dir, output_dir, multimer_model_dir="",
        ...
        for method in os.listdir(monomer_model_dir):
            if targetname not in os.listdir(monomer_model_dir + '/' + method):
                continue
            for i in range(0, 5):
                os.system(f"cp {monomer_model_dir}/{method}/{targetname}/ranked_{i}.pdb {pdbdir}/{method}_{i}.pdb")
                os.system(
                    f"cp {monomer_model_dir}/{method}/{targetname}/result_model_{i + 1}.pkl {pkldir}/{method}_{i}.pkl")
