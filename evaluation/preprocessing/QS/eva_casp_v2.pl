...
while(<TLIST>){
	...
	while(($key, $value) = each(%rank_1)) {
		...
		my $model = $target_id . $key . '_1';
		if (!-e $casp_models_dir.'/'.$target_id.'/'.$model)
		{
			$model = $target_id . $key . '_1o';
		}
		#the number 1 model;
		#print "3\n";
		#print $temp_dir."\n";
		...
		while (<POUT>) {
			...
			if (substr($line, 0, 8) eq "TM-score" and index($line, "length of Structure_1") > 0) {
	...
	while (($key, $value) = each(%rank_5)) {
		...
		my $highest_SUB = 0;
		while ($i <= 5) {
			my $model = $target_id . $key . '_' . $i;
			if (!-e $casp_models_dir.'/'.$target_id.'/'.$model)
			{
				$model = $target_id . $key . '_'. $i . 'o';
			}
