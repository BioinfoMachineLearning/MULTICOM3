...
while($counter < $length){
	...
	my $file_new = "$pdb_ids[$counter].pdb";
	my $file_unzipped =  "$native_pdb_dir/$pdb_ids[$counter].pdb";
	print $file_unzipped."\n";
	...
	if(-e "$file_unzipped")
	{
		print "$file_unzipped is found~!\n";
	}else{
		print "Downloading $file_unzipped from pdb ftp\n";
		print("wget ftp://ftp.wwpdb.org/pub/pdb/data/structures/all/pdb/$file_unzipped\n\n");   # "##" commented lines should be removed if you want to run the program.
		system("wget ftp://ftp.wwpdb.org/pub/pdb/data/structures/all/pdb/$file_unzipped");   # "##" commented lines should be removed if you want to run the program.
      	}
	#`mv $file $path/$file_new`;   #copy the ftp downloaded pdb original files into ./pdb_orig
    `cp $file_unzipped $path/$file_new`;   #copy the ftp downloaded pdb original files into ./pdb_orig
	my $pdb_orig_seq_file = "$target_ids[$counter]".'_pdb_orig_seq';
	my $status = system("perl check_chain_num.pl $path/$file_new"); #Added by jie to check if pdb file has multiple chain, because in this case we don't know wether casp sequence is matched to which chains
	if($status)
	{
		print "\n\n !!!!!!!!!!! Warning: Pdb file $file_new has multiple chains, please remove it!\n\n\n";
	}
	...
	while(1)
	{
        ...
        `perl pdb2seq.pl $path/$file_new > $path/$pdb_orig_seq_file`; #parse the sequence from the downloaded pdb files;
		print("perl pdb2seq.pl $path/$file_new > $path/$pdb_orig_seq_file\n");
