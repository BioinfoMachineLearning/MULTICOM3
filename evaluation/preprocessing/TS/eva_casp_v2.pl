...
foreach my $target (@targets){
	...
	foreach my $model (@models){
		...
		print $model;
		my $group = substr($model, index($model, "TS"), 5);#substr($model, 0, (length($model) - 4));
...
while(<TLIST>){
	...
	while(($key, $value) = each(%rank_1)){    #get the rank of rank_1 first, ie the number 1 model;
		...
		#print $casp_models_dir.$target_id."\n";
			...
			if($model eq $name ){    #the number 1 model;
				...
				#print "perl pre2zhang.pl $casp_models_dir/$target_id/$model $pdb_filtered_file $model_filtered\n";
					`perl pre2zhang.pl $casp_models_dir/$target_id/$model $pdb_filtered_file $model_filtered`;
				}
	...
	while(($key, $value) = each(%rank_5)){    #get the sum of the best model in 5 models;
		...
		my $highest_SUB = 0;
		while($i <= 5){
			my $name = $target_id.$key.'_'.$i;
			#print $name."\n";
			#sleep 2;
			opendir(DIR, "$casp_models_dir/$target_id") or print  "cannot open dir! predictions mayben missing in this target.\n";
			my @models = readdir DIR;
			closedir DIR;
			foreach my $model (@models){
				if($model eq "." || $model eq ".."){
