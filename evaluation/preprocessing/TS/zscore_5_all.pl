...
open(OUT, ">$outf/zscore_5.txt") || die("Couldn't open file $outf/zscore_5.txt\n");
open(OUT2, ">$outf/z_5.log") || die("Couldn't open file $outf/z_5.log\n");

opendir ( DIR3, "$infolder" ) || die "Error in opening dir $infolder\n";
while(($ln3 = readdir(DIR3))){
	if (length($ln3)>2 && ( substr($ln3,0,2) eq "T0" or substr($ln3,0,2) eq "T1" )) {
		open(IN1, "$infolder/$ln3") || die("Couldn't open file $infolder/$ln3\n"); 
		@a=<IN1>;
		close IN1;

		$name=substr($ln3,0,index($ln3,"."))."_zscore_5.txt";
