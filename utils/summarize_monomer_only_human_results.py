...
if __name__ == '__main__':
    ...
    ranking_csvs = {'af_apollo_avg': f"{qa_dir}/{target}/pairwise_af_avg.ranking",
                    'af': f"{qa_dir}/{target}/alphafold_ranking.csv",
                    'apollo': f"{qa_dir}/{target}/pairwise_ranking.tm",
                    'deeprank': f"{qa_dir}/{target}/casp13/Full_TS/eva/HumanQA_gdt_prediction.txt",
                    'deeprank_dncon4': f"{qa_dir}/{target}/casp14/Full_TS/eva/HumanQA_gdt_prediction.txt",
                    'deeprank3_cluster': f"{qa_dir}/{target}/casp14/Full_TS/eva_DeepRank3/DeepRank3_Cluster.txt",
                    'deeprank3_singleqa': f"{qa_dir}/{target}/casp14/Full_TS/eva_DeepRank3/DeepRank3_SingleQA.txt",
                    'deeprank3_singleqa_lite': f"{qa_dir}/{target}/casp14/Full_TS/eva_DeepRank3/DeepRank3_SingleQA_lite.txt",
                    'enqa': f"{qa_dir}/{target}/enqa_ranking.csv",
                    'modfoldclust2': f"{qa_dir}/{target}/casp13/Full_TS/eva/ALL_scores/modfoldclust2.{target}"}
    ...
    for method in ['pcons', 'DeepQA', 'dfire', 'dncon2_long-range', 'dncon2_medium-range', 'dncon2_short-range',
                   ...
                   'ss_sim']:
        ranking_csvs[method] = f"{qa_dir}/{target}/casp13/Full_TS/eva/ALL_scores/feature_{method}.{target}"
    ...
    for index, method in enumerate(find_res):
        ...
        for i in range(len(global_scores)):
            if global_scores.loc[i, f'model{index}'].find('_A.pdb') > 0 or \
                    global_scores.loc[i, f'model{index}'].find('_B.pdb') > 0:
                continue
            keep_indices += [i]

        print(keep_indices)
        print(global_scores.iloc[keep_indices])
        all_dfs += [global_scores.iloc[keep_indices]]
