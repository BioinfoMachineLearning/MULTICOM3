...
if __name__ == '__main__':
    ...
    inpdbdir = args.indir + '/pdb'
    inpkldir = args.indir + '/pkl'

    outpdbdir = args.outdir + '/pdb'
    makedir_if_not_exists(outpdbdir)
    outpkldir = args.outdir + '/pkl'
    ...
    for inpdb in os.listdir(inpdbdir):
        if inpdb.find('.pdb') < 0:
            continue
        chain_contents = {}
        prev_chain_name = ""
        ...
        for line in open(inpdbdir + '/' + inpdb, 'r').readlines():
        ...
        with open(outpdbdir + '/' + inpdb, 'w') as fw:
        ...
        extract_pkl(inpkl=f"{inpkldir}/{inpdb.replace('.pdb', '.pkl')}",
                    outpkl=f"{outpkldir}/{inpdb.replace('.pdb', '.pkl')}",
                    indices=chain_domain_indices)
